...
exports.findall = (req, res) => {
    ...
    if (page || status) {
        if (page && !status) {
            Work.findAndCountAll({ order: [["id", "DESC"]], offset: soLuongBoQua, limit: PAGE_SIZE, include: [Company] }).then(data => {
                res.json({ data: data })
            }).catch(er => {
                throw er;
            })
        } else if (status && !page) {
            Work.findAndCountAll({ where: { status: status }, order: [["id", "DESC"]], include: [Company] }).then(data => {
                res.json({ data: data })
            }).catch(er => {
                throw er;
            })
        } else {
            Work.findAndCountAll({ where: { status: status }, order: [["id", "DESC"]], offset: soLuongBoQua, limit: PAGE_SIZE, include: [Company] }).then(data => {
                res.json({ data: data })
            }).catch(er => {
                throw er;
            })
        ...
        Work.findAndCountAll({ order: [["id", "DESC"]], include: [Company] }).then(data => {
            res.json({ data: data })
        }).catch(er => {
            throw er;
        })
...
exports.search = (req, res) => {
    ...
    Work.findAndCountAll({ where: {nature: { [Op.like]: `%${nature}%` }, address: { [Op.like]: `%${address}%` }, name: { [Op.like]: `%${name}%` }, status: status }, order: [["id", "DESC"]], attributes: ['id', "name", 'address', 'createdAt', 'price1', 'price2'], include: [{ model: Company, attributes: ['name', 'id', 'avatar'] }] }).then(data => {
        res.json({ data: data })
    }).catch(er => {
        throw er;
    })
